...
export default function (eleventyConfig) {
  ...
  // The sections in the order they sit on the scale: off-scale "discrete"
  // first (below resolution), then everything on the ruled bar from smallest
  // to largest, then "unbound" (past the far end).
  const inScaleOrder = (list) => {
    const zoneRank = (s) => (s.zone === "discrete" ? 0 : s.zone === "unbound" ? 2 : 1);
    return (list || [])
      .filter((s) => !s.hidden)
      .sort((a, b) => zoneRank(a) - zoneRank(b) || Number(a.at || 0) - Number(b.at || 0));
  };

  // Where you are and what sits either side of you, for the header.
  // Most sites can't offer a meaningful "next section" because their
  // sections aren't in any order. This one's are — that ordering is the
  // whole idea — so you can walk the site from small to large.
  //
  // No wrapping at the ends: the first section has nothing smaller and says
  // so by leaving the link out. Looping back round would suggest the scale
  // joins up, which is the opposite of what the site is about.
  eleventyConfig.addFilter("scaleNeighbours", (list, key) => {
    const ordered = inScaleOrder(list);
    const i = ordered.findIndex((s) => s.key === key);
    if (i === -1) return { current: null, smaller: null, larger: null };
    return {
      current: ordered[i],
      smaller: ordered[i - 1] || null,
      larger: ordered[i + 1] || null,
    };
  });
